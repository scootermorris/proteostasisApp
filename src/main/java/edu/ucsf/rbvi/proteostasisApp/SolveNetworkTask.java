...
public class SolveNetworkTask extends AbstractTask {
    ...
    private JsonObject buildRequest(CyNetwork network, Map<String, CyNode> nodeNameMap) {
        ...
        for (CyNode cyNode : network.getNodeList()) {
            ...
            String nodeClass = Utils.getStr(row,LoadNetworkTask.COL_NODE_CLASS);
            ...
            Double totalNm = Utils.getDbl(row, LoadNetworkTask.COL_TOTAL_NM);
        ...
        for (CyEdge cyEdge : network.getEdgeList()) {
            ...
            String srcClass = Utils.getStr(network.getRow(cyEdge.getSource()),LoadNetworkTask.COL_NODE_CLASS);
            String tgtClass = Utils.getStr(network.getRow(cyEdge.getTarget()),LoadNetworkTask.COL_NODE_CLASS);
            if ("cluster".equals(srcClass) || "cluster".equals(tgtClass)) continue;

            CyRow edgeRow   = network.getRow(cyEdge);
            String edgeClass = Utils.getStr(edgeRow,LoadNetworkTask.COL_EDGE_CLASS);
            Double kdNm      = Utils.getDbl(edgeRow,LoadNetworkTask.COL_KD_NM);
